...
- **Source** = the GoodBehavior bundle this skill ships in — the repo/dir that contains `.omp/skills/*-goodbehavior/`,
  `.omp/extensions/done-gate.ts` (a session_stop extension auto-discovered by OMP — no settings wiring anywhere),
  and `templates/`. Normally the current session's project. Record its absolute path, and
  if it's a git repo its HEAD (`git -C <source> rev-parse HEAD`) — that becomes the merge base for `/skill:update-goodbehavior`.
...
- The target need not be a git repo to install, but note: `/skill:update-goodbehavior` later needs the *source* committed
  (for a merge base), not the target.
...
the project's *kind* from file types); use `ask` for the load-bearing ones:
...
- **Existing context docs** — `AGENTS.md`, `HANDOFF.md`, `CONTRIBUTING`, `README`, `docs/`, any
...
Present a concrete adoption plan grounded in Phases 1 + 2, then ask the user to choose. Use `ask` for the
...
- **Principles home** — new `AGENTS.md` · append to an existing AGENTS.md · or link the principles from the existing
...
- **Done-gate extension** — install it (project-local) vs off. (Skills and the gate are **always project-local
  copies** — never a symlink, never a global `~/.omp/` install — so this is a yes/no, not a scope choice.)
...
Install location is **not** a question: everything goes into the **target's** own `.omp/` as a copy, by design (a
project adapts its copy to its stack without touching other projects or your global config; `/skill:update-goodbehavior`
...
1. **Run the installer.** Write the confirmed choices as a plan JSON and execute it:
   ...
   manifest written at `<target>/.omp/goodbehavior/manifest.json` with sourceCommit + a sha256 per file as installed
   (warns if the source has no commits — update can't 3-way-merge until it does). Re-runs are no-ops. **Note
   `adopt-goodbehavior` itself is NOT shipped** — it's a run-once installer; post-install the target only needs the
   working skills plus `/skill:update-goodbehavior`. Relay the script's created/skipped/warnings report.
...
3. **`project-profile` memory** (judgment): write the resolved slots — *the-real-thing / verify / evidence* and each
   ...
   `/skill:verify-goodbehavior` and `/skill:gate-build-goodbehavior` read "done" from it. For a custom fallback, also
   write the profile body into `<target>/.omp/goodbehavior/profiles/custom.md` (no core file to copy).
4. **Confirmed dev approach** (judgment; *only if a development slot is in play*): record in the target's AGENTS.md
...
## Report
List exactly what was created / amended / skipped **in the target**, and where principles, skills, the done-gate
extension, plans, memory, and the manifest now live. Note that the other commands (`/skill:audit-goodbehavior`,
`/skill:roadmap-goodbehavior`, `/skill:gate-build-goodbehavior`, `/skill:verify-goodbehavior`,
`/skill:learn-goodbehavior`, `/skill:update-goodbehavior`) now run from a session opened **in the target project** —
its `.omp/skills/` holds the copies. Do NOT claim the method "works" — the scaffolding is in place; it proves itself
